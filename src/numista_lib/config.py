"""Configuration settings for numista-lib.
...
if TYPE_CHECKING:
    from numista_lib.client import NumistaClientAsync, NumistaClientSync
...
def get_environment_file() -> Path | None:
    ...
    3. .numista_lib.env in home directory
    4. /etc/numista_lib.env
    ...
    home_env = Path.home() / ".numista_lib.env"
    if home_env.is_file():
        return home_env

    etc_env = Path("/etc/numista_lib.env")
    if etc_env.is_file():
        return etc_env
...
class Settings(BaseSettings):
    ...
    cache_dir: Path = Field(
        default_factory=lambda: Path.home() / ".cache/numista-lib/hishel",
...
def create_client_from_settings(settings: Settings) -> NumistaClientSync:
    ...
    """
    from numista_lib.client import NumistaClientSync
...
def create_async_client_from_settings(settings: Settings) -> NumistaClientAsync:
    ...
    """
    from numista_lib.client import NumistaClientAsync
